...
console.log('$ mimir-dev-build-ts', process.argv.slice(2).join(' '));

// webpack build
function buildWebpack() {
  const config = WP_CONFIGS.find((c) => fs.existsSync(path.join(process.cwd(), c)));

  execSync(`yarn mimir-exec-webpack --config ${config} --mode production`);
}
...
function orderPackageJson(repoPath, dir, json) {
  json.author = 'dev.mimir';
  json.bugs = `https://github.com/${repoPath}/issues`;
  json.homepage = json.homepage || `https://github.com/${repoPath}${dir ? `/tree/master/${dir}` : ''}#readme`;
...
async function buildJs(repoPath, dir) {
  ...
  if (!fs.existsSync(path.join(process.cwd(), '.skip-build'))) {
    const srcHeader = `// Copyright 2023-${new Date().getFullYear()} dev.mimir authors & contributors\n// SPDX-License-Identifier: Apache-2.0\n`;
...
async function main() {
  ...
  execSync('yarn mimir-exec-tsc --build tsconfig.build.json');
